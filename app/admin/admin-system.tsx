...
import { ButtonSpinner, ChoirLoader } from "@/app/loader";
...
type DayAvailability = { answered: number; notAnswered: number; unfinishedDrafts: number; days: Array<{ key: string; label: string; count: number; missing: string[] }> };
...
type Detail = { registration: { id: string; name: string; parents: { fatherName?: string; fatherPhone?: string; motherName?: string; motherPhone?: string; email?: string }; paymentMethod?: string; availableDays?: string; reviewStatus: string; proofStatus: string; studentStatus?: string; enrollmentId?: string; groupId?: string; privateNotes: string; care?: CareDetail | null }; payments: Array<{ id: string; periodKey: string; label: string; amountDue: number; amountPaid: number; status: string; paid: boolean; method?: string }>; history: Array<{ id: number; summary?: string; action: string; created_at: string }> };
...
export default function AdminSystem() {
  ...
  const loadYear = useCallback(async (selectedYearId: string) => {
    ...
    try {
      const [registrationData, groupData, scheduleData, methodData, linkData, historyData] = await Promise.all([
        ...
        readJson<{ groups: Group[]; unassigned: Unassigned[]; dayAvailability: DayAvailability | null }>(`/api/admin/groups?yearId=${encodeURIComponent(selectedYearId)}`),
      ...
      setGroups(groupData.groups); setUnassigned(groupData.unassigned); setDayAvailability(groupData.dayAvailability || null); setEvents(scheduleData.events); setVersions(scheduleData.versions);
  ...
  return <main className="admin-shell admin-system">
    ...
    <section className="admin-main admin-workspace"><header className="admin-header"><div><div className="admin-title-line"><h1>{navigation.find((item) => item.id === tab)?.label}</h1><span>Live system</span></div><p>{loading ? "Loading the latest records…" : `${activeYear?.name || "School year"} · changes are saved to the private database`}</p></div><div className="admin-header-actions"><select aria-label="School year" value={yearId} onChange={(event) => setYearId(event.target.value)}>{years.map((year) => <option key={year.id} value={year.id}>{year.name}{year.status === "active" ? " · current" : ""}</option>)}</select><button className="admin-primary" onClick={() => setAddOpen(true)}>＋ Add student</button></div></header>
      {loading && !years.length ? <ChoirLoader variant="page" label="Loading the administrator system…" /> : <div className={loading ? "admin-panel is-refreshing" : "admin-panel"}>
      ...
      {tab === "groups" && <GroupsPanel groups={groups} unassigned={unassigned} dayAvailability={dayAvailability} yearId={yearId} mutate={mutate} reload={reload} toast={setToast} />}
...
function AgreementsPanel({ yearId, year, years, mutate, toast }: { yearId: string; year?: Year; years: Year[]; mutate: <T>(url: string, body: Record<string, unknown>, method?: string) => Promise<T>; toast: (message: string) => void }) {
  ...
  return <div className="admin-page-stack">
    <section className="admin-card">
      <div className="admin-card-head"><div><p className="eyebrow">Version controlled</p><h2>{year?.name || "School year"} agreement</h2></div><button className="admin-primary" disabled={loading || publishing || !sections.length} onClick={() => void publish()}>{publishing ? <><ButtonSpinner /> Publishing…</> : "Publish new version"}</button></div>
    ...
    {loading ? <section className="admin-card"><ChoirLoader variant="panel" label="Loading the agreement…" /></section> : sections.map((section, sectionIndex) => <section className="admin-card admin-form-card" key={`${section.title}-${sectionIndex}`}>
