...
type EmailItem = { id: string; template: string; status: string; attempts: number; error?: string; createdAt: string };
...
export default function AdminSystem() {
  ...
  useEffect(() => { let active = true; Promise.all([readJson<{ years: Year[] }>("/api/admin/years"), readJson<{ backups: Backup[] }>("/api/admin/backups"), readJson<{ configured: boolean; emails: EmailItem[] }>("/api/admin/email")]).then(([yearData, backupData, emailData]) => { if (!active) return; const loadedYears = yearData.years; const selectedYear = loadedYears.find((year) => year.status === "active") || loadedYears[0]; setYears(loadedYears); setBackups(backupData.backups); setEmailConfigured(emailData.configured); setEmailItems(emailData.emails); if (selectedYear) setYearId(selectedYear.id); }).catch((error) => active && setToast(error instanceof Error ? error.message : "Administrator data could not be loaded.")); return () => { active = false; }; }, []);
  ...
  return <main className="admin-shell admin-system">
    ...
    <section className="admin-main admin-workspace"><header className="admin-header"><div><div className="admin-title-line"><h1>{navigation.find((item) => item.id === tab)?.label}</h1><span>Live system</span></div><p>{loading ? "Loading the latest records…" : `${activeYear?.name || "School year"} · changes are saved to the private database`}</p></div><div className="admin-header-actions"><select aria-label="School year" value={yearId} onChange={(event) => setYearId(event.target.value)}>{years.map((year) => <option key={year.id} value={year.id}>{year.name}{year.status === "active" ? " · current" : ""}</option>)}</select><button className="admin-primary" onClick={() => setAddOpen(true)}>＋ Add student</button></div></header>
      ...
      {tab === "settings" && <SettingsPanel year={activeYear} yearId={yearId} backups={backups} emailConfigured={emailConfigured} emails={emailItems} reloadEmails={async () => { const data = await readJson<{ configured: boolean; emails: EmailItem[] }>("/api/admin/email"); setEmailConfigured(data.configured); setEmailItems(data.emails); }} reloadBackups={async () => { const data = await readJson<{ backups: Backup[] }>("/api/admin/backups"); setBackups(data.backups); }} signOut={signOut} toast={setToast} />}
    </section>
...
function StudentDrawer({ selected, detail, notes, setNotes, close, action, setPaymentStatus, confirmPayment, groups, yearName, mutate, reload, toast }: { selected: Student; detail: Detail | null; notes: string; setNotes: (value: string) => void; close: () => void; action: (action: "approve" | "reopen" | "archive" | "restore" | "notes") => Promise<void>; setPaymentStatus: (id: string, paid: boolean) => Promise<void>; confirmPayment: (value: { id: string; label: string }) => void; groups: Group[]; yearName: string; mutate: <T>(url: string, body: Record<string, unknown>, method?: string) => Promise<T>; reload: (message?: string) => Promise<void>; toast: (message: string) => void }) {
  ...
  async function changeGroup(groupId: string) { if (!detail?.registration.enrollmentId) return; await mutate("/api/admin/groups", { action: "assign", enrollmentId: detail.registration.enrollmentId, groupId }, "PATCH"); await reload("Group assignment updated. The parent’s private schedule link now follows this group."); }
...
function GroupsPanel({ groups, unassigned, yearId, mutate, reload, toast }: { groups: Group[]; unassigned: Unassigned[]; yearId: string; mutate: <T>(url: string, body: Record<string, unknown>, method?: string) => Promise<T>; reload: (message?: string) => Promise<void>; toast: (message: string) => void }) {
  ...
  async function create() { try { await mutate("/api/admin/groups", { ...form, yearId, weekday: Number(form.weekday), ageMin: form.ageMin || null, ageMax: form.ageMax || null }); setForm({ name: "", weekday: "3", startTime: "17:00", endTime: "17:50", location: "", ageMin: "", ageMax: "" }); await reload("Group created with a private parent schedule link."); } catch (error) { toast(error instanceof Error ? error.message : "Group could not be created."); } }
  ...
  async function removeGroup(group: Group) { if (!window.confirm(`Delete ${group.name}? Its private parent link will stop working. Groups with assigned students cannot be deleted.`)) return; try { await readJson(`/api/admin/groups?id=${encodeURIComponent(group.id)}`, { method: "DELETE" }); await reload(`${group.name} deleted.`); } catch (error) { toast(error instanceof Error ? error.message : "Group could not be deleted."); } }
...
function SettingsPanel({ year, yearId, backups, emailConfigured, emails, reloadEmails, reloadBackups, signOut, toast }: { year?: Year; yearId: string; backups: Backup[]; emailConfigured: boolean; emails: EmailItem[]; reloadEmails: () => Promise<void>; reloadBackups: () => Promise<void>; signOut: () => Promise<void>; toast: (message: string) => void }) {
  const [proofRequired, setProofRequired] = useState(year?.proofRequired ?? true); const [cashReminderText, setCashReminderText] = useState(""); const [administratorEmail, setAdministratorEmail] = useState(""); const [savedEmail, setSavedEmail] = useState(""); const [newPasscode, setNewPasscode] = useState(""); const [currentPasscode, setCurrentPasscode] = useState(""); const [retrying, setRetrying] = useState(false);
  useEffect(() => { readJson<{ proofUploadRequired: boolean; cashReminderText: string; administratorEmail: string }>(`/api/admin/settings?yearId=${encodeURIComponent(yearId)}`).then((data) => { setProofRequired(data.proofUploadRequired); setCashReminderText(data.cashReminderText); setAdministratorEmail(data.administratorEmail); setSavedEmail(data.administratorEmail); }).catch((error) => toast(error instanceof Error ? error.message : "Settings could not be loaded.")); }, [toast, yearId]);
  const needsCurrentPasscode = Boolean(newPasscode) || administratorEmail.trim().toLowerCase() !== savedEmail.trim().toLowerCase();
  async function save() { try { if (needsCurrentPasscode && !currentPasscode) { toast("Enter the current passcode to change the administrator email or passcode."); return; } await readJson("/api/admin/settings", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ yearId, proofUploadRequired: proofRequired, cashReminderText, administratorEmail, newPasscode, currentPasscode }) }); setSavedEmail(administratorEmail); setNewPasscode(""); setCurrentPasscode(""); toast("Administrator and registration settings saved."); } catch (error) { toast(error instanceof Error ? error.message : "Settings could not be saved."); } }
  async function retryEmails(id?: string) { if (retrying) return; setRetrying(true); try { await readJson("/api/admin/email", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(id ? { id } : { all: true }) }); await reloadEmails(); toast(id ? "Delivery retried for the selected message." : "Delivery retried for all queued messages."); } catch (error) { toast(error instanceof Error ? error.message : "Email delivery could not be retried."); } finally { setRetrying(false); } }
  ...
  return <div className="admin-page-stack"><section className="admin-card admin-form-card"><div className="admin-card-head"><div><p className="eyebrow">Administrator</p><h2>Access & recovery</h2></div><p>Sessions log out after inactivity. The hidden corner only opens sign-in; the passcode protects access.</p></div><div className="compact-form-grid settings-form"><label><span>Administrator / recovery email</span><input type="email" value={administratorEmail} onChange={(event) => setAdministratorEmail(event.target.value)} /></label><label><span>New passcode</span><input type="password" minLength={4} value={newPasscode} onChange={(event) => setNewPasscode(event.target.value)} placeholder="Leave blank to keep it" /><small>Four characters minimum. A longer private code is strongly recommended.</small></label>{needsCurrentPasscode && <label><span>Current passcode</span><input type="password" value={currentPasscode} onChange={(event) => setCurrentPasscode(event.target.value)} placeholder="Required to confirm this change" /><small>Changing the administrator email or passcode requires the current passcode.</small></label>}<label className="wide"><span>Cash responsibility reminder</span><textarea value={cashReminderText} onChange={(event) => setCashReminderText(event.target.value)} /><small>This editable text appears after Cash is selected. It is not hardcoded in the registration screen.</small></label><label className="switch-field"><span>Default payment proof required</span><button className={proofRequired ? "setting-toggle on" : "setting-toggle"} role="switch" aria-checked={proofRequired} onClick={() => setProofRequired((current) => !current)}><span /></button></label><button className="admin-primary" onClick={() => void save()}>Save settings</button><button className="secondary-button" onClick={() => void signOut()}>Log out now</button></div></section><section className="admin-card"><div className="admin-card-head"><div><p className="eyebrow">Backups</p><h2>Protected records</h2></div><div className="card-actions"><button onClick={() => void backup()}>Create encrypted backup</button><a href="/api/admin/backups?download=1" download>Download JSON</a></div></div><p className="admin-muted">A protected server snapshot is created daily and after significant changes, no more than once every five minutes. Restore always creates one more safety snapshot first.</p><div className="backup-list">{backups.slice(0, 8).map((item) => <div key={item.id}><span><strong>{item.reason.replaceAll("-", " ")}</strong><small>{(item.byteSize / 1024).toFixed(1)} KB</small></span><time>{new Date(item.createdAt).toLocaleString("en-GB")}</time><button className="text-link" onClick={() => void restore(item)}>Restore</button><b>{item.status}</b></div>)}</div></section><section className="admin-card"><div className="admin-card-head"><div><p className="eyebrow">Email delivery</p><h2>{emailConfigured ? "Email service connected" : "Email provider not connected"}</h2></div><span className={`status-chip ${emailConfigured ? "status-approved" : "status-awaiting-approval"}`}>{emailConfigured ? "Ready" : "Setup needed"}</span></div><p className="admin-muted">Signed-agreement, schedule and save-and-continue emails are queued automatically. Actual delivery begins when a verified sender and email-provider key are configured.</p>{emailConfigured && emails.some((item) => item.status === "pending" || item.status === "failed") && <div className="card-actions"><button className="secondary-button" disabled={retrying} onClick={() => void retryEmails()}>{retrying ? "Retrying…" : "Retry queued messages"}</button></div>}<div className="email-status-list">{emails.slice(0, 8).map((item) => <div key={item.id}><span>{item.template}</span><b>{item.status}</b><small>{item.error || new Date(item.createdAt).toLocaleString("en-GB")}</small>{emailConfigured && (item.status === "pending" || item.status === "failed") && <button className="text-link" disabled={retrying} onClick={() => void retryEmails(item.id)}>Retry</button>}</div>)}</div></section></div>;
}
